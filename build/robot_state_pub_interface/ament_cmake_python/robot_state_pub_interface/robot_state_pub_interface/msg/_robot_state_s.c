...
bool robot_state_pub_interface__msg__robot_state__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("robot_state_pub_interface.msg._robot_state.RobotState", full_classname_dest, 53) == 0);
  }
  robot_state_pub_interface__msg__RobotState * ros_message = _ros_message;
  {  // joint_names
  ...
  {  // set_effort
    PyObject * field = PyObject_GetAttrString(_pymsg, "set_effort");
    if (!field) {
      return false;
    }
    if (PyObject_CheckBuffer(field)) {
      ...
      double * dest = ros_message->set_effort.data;
      rc = PyBuffer_ToContiguous(dest, &view, view.len, 'C');
      if (rc < 0) {
        PyBuffer_Release(&view);
        Py_DECREF(field);
      ...
      PyObject * seq_field = PySequence_Fast(field, "expected a sequence in 'set_effort'");
      if (!seq_field) {
        Py_DECREF(field);
        return false;
      }
...
PyObject * robot_state_pub_interface__msg__robot_state__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("robot_state_pub_interface.msg._robot_state");
  ...
  {  // set_effort
    PyObject * field = NULL;
    field = PyObject_GetAttrString(_pymessage, "set_effort");
    if (!field) {
      return NULL;
    }
    assert(field->ob_type != NULL);
    ...
    if (ros_message->set_effort.size > 0) {
      // populating the array.array using the frombytes method
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      double * src = &(ros_message->set_effort.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->set_effort.size * sizeof(double));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
