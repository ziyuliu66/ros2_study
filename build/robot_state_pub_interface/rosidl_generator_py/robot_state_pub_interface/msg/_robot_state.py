...
class Metaclass_RobotState(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            module = import_type_support('robot_state_pub_interface')
        except ImportError:
            import logging
            import traceback
            logger = logging.getLogger(
                'robot_state_pub_interface.msg.RobotState')
            logger.debug(
...
class RobotState(metaclass=Metaclass_RobotState):
    ...
    def __init__(self, **kwargs):
        ...
        self.set_effort = array.array('d', kwargs.get('set_effort', []))
    ...
    def __eq__(self, other):
        ...
        if self.angles != other.angles:
            return False
        if self.set_effort != other.set_effort:
            return False
        return True
    ...
    @builtins.property
    def set_effort(self):
        """Message field 'set_effort'."""
        return self._set_effort

    @set_effort.setter
    def set_effort(self, value):
        if isinstance(value, array.array):
            assert value.typecode == 'd', \
                "The 'set_effort' array.array() must have the type code of 'd'"
            self._set_effort = value
            return
        if __debug__:
            from collections.abc import Sequence
            from collections.abc import Set
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 all(not (val < -1.7976931348623157e+308 or val > 1.7976931348623157e+308) or math.isinf(val) for val in value)), \
                "The 'set_effort' field must be a set or sequence and each value of type 'float' and each double in [-.000000, .000000]"
        self._set_effort = array.array('d', value)
